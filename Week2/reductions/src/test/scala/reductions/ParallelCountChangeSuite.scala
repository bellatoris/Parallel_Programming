...
//  def countChangeForTest(money:Int, coin: List[Int]) = countChange(money, coin)
  def countChangeForTest(money:Int, coin: List[Int]) =
    parCountChange(money, coin, totalCoinsThreshold(coin.length))

  test("countChange should return 0 for money < 0") {
    def check(money: Int, coins: List[Int]) = 
      assert(countChangeForTest(money, coins) == 0,
  ...
  test("countChange should return 1 when money == 0") {
    def check(coins: List[Int]) = 
      assert(countChangeForTest(0, coins) == 1,
  ...
  test("countChange should return 0 for money > 0 and coins = List()") {
    def check(money: Int) = 
      assert(countChangeForTest(money, List()) == 0,
  ...
  test("countChange should work when there is only one coin") {
    def check(money: Int, coins: List[Int], expected: Int) =
      assert(countChangeForTest(money, coins) == expected,
