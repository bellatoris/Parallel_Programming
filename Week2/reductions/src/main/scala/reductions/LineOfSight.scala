...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    def nested(idx: Int, max: Float): Unit = {
      if (idx != input.length) {
        val tangent = if (idx == 0) 0 else input(idx) / idx
        if (max < tangent) output(idx) = tangent
        else output(idx) = max
        nested(idx + 1, output(idx))
      }
    }
    nested(0, 0)
  }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    def nested(idx: Int, max: Float): Float = {
      if (idx == until) max
      else if (max < input(idx)/idx) nested(idx+1, input(idx)/idx)
      else nested(idx+1, max)
    }
    nested(from, input(from)/from)
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    def nested(idx: Int, max: Float): Unit = {
      if (idx != until) {
        if (max < input(idx)/idx) output(idx) = input(idx)/idx
        else output(idx) = max
        nested(idx+1, output(idx))
      }
    }
    nested(from, startingAngle)
  }
